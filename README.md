...
完整协议见
[`doc/接口规范/8路640x480视频在4K30_HDMI中的空间封装规范.md`](doc/接口规范/8路640x480视频在4K30_HDMI中的空间封装规范.md)。
...
| [`doc/`](doc/README.md) | 按架构、接口、实现、验证、问题和需求分类的项目文档 |

视频采集、帧快照和双格式硬件预处理的历史阶段基线见
[`doc/架构设计/当前已实现视频采集与AI输入架构.md`](doc/架构设计/当前已实现视频采集与AI输入架构.md)。

包含当前双 Gemmini16 SoC、FBus、双 worker TinyYOLOv2、结果管理和检测框叠加的
完整架构见
[`doc/架构设计/当前系统完整架构.md`](doc/架构设计/当前系统完整架构.md)。该文档以当前 RTL 和裸机
软件为准，并将当前 Gemmini 计算异常单独列为已知问题。
