...
parser.add_argument('--collateral', type=Path, default=None)
...
parser.add_argument('--read-ids', type=int, choices=[2, 4, 8, 16, 18, 31])
...
if args.collateral is None:
    manifest = (repo / 'build/soc_manifest.tcl').read_text()
    config = re.search(r'^\s*(tsmc\S+Config)\s*$', manifest, re.M)
    if not config:
        raise SystemExit('Cannot find SOC_CONFIG in build/soc_manifest.tcl')
    args.collateral = repo / 'generated/soc' / config[1] / 'gen-collateral'
...
# Keep the production 18/14 ID partition when read and write traffic coexist.
...
read_id_count = args.read_ids or ((1 << axi_id_width) if args.shared_write_id else first_write_id)
max_read_ids = (1 << axi_id_width) if args.shared_write_id or not write_traffic else first_write_id
...
print(f"FBUS_TEST_CONFIG fbus_bits={memory_width} axi_id_bits={axi_id_width} "
      ...
      f"base=0x{args.base_address:x} burst={args.burst or 'legacy'} "
...
with tempfile.TemporaryDirectory(prefix='fbus-generated-') as directory:
    ...
    cmd = [verilator, '--binary', '--timing', '--assert', '-O0', '-Wno-fatal',
           '--top-module', 'tb_fbus_generated_path', f'-GREADER_SLOTS={args.slots}', f'-GBYTES={args.bytes}', f"-GBASE_ADDR=33'h{args.base_address:09x}", f"-GSHORT_ONLY=1'b{int(args.short_only)}", f'-GMIN_MBPS={args.min_mbps}', f"-GFAIR_MEMORY=1'b{int(args.fair_memory)}", f"-GWRITE_TRAFFIC=1'b{int(write_traffic)}", f"-GWRITE_ONLY=1'b{int(args.write_only)}", f'-GWRITE_COUNT={args.write_count}', f'-GAXI_ID_WIDTH={axi_id_width}', f'-GREAD_ID_COUNT={read_id_count}', f'-GFIRST_WRITE_ID={first_write_id}', f'-GWRITE_ID_COUNT={write_id_count}',
