...
// Loop-matmul variant for an A operand already resident in the selected
// Scratchpad partition.  The lda started/completed bits suppress only A's
// DRAM load; B, bias, accumulator and store paths remain enabled.
#define gemmini_loop_ws_skip_a(I, J, K, pad_I, pad_J, pad_K, A, B, D, C, A_stride, B_stride, D_stride, C_stride, A_transpose, B_transpose, full_C, low_D, ex_accumulate, act, a_spad_id, b_spad_id, is_resadd) \
  { \
    ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, ((uint64_t)(pad_K) << 32) | ((uint64_t)(pad_J) << 16) | (uint64_t)(pad_I), ((uint64_t)(K) << 32) | ((uint64_t)(J) << 16) | (uint64_t)(I), k_LOOP_WS_CONFIG_BOUNDS) \
    ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, (A), (B), k_LOOP_WS_CONFIG_ADDRS_AB) \
    ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, (D), (C), k_LOOP_WS_CONFIG_ADDRS_DC) \
    ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, (A_stride), (B_stride), k_LOOP_WS_CONFIG_STRIDES_AB) \
    ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, (D_stride), (C_stride), k_LOOP_WS_CONFIG_STRIDES_DC) \
    ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, ((uint64_t)(a_spad_id) << 18) | ((uint64_t)(b_spad_id) << 16) | ((uint64_t)(act) << 8) | ((low_D) << 2) | ((full_C) << 1) | (ex_accumulate), (1ULL << 3) | ((is_resadd) << 2) | ((B_transpose) << 1) | (A_transpose), k_LOOP_WS) \
  }
...
*/
  ...
  // A NULL pointer is reserved for the Stage 8F resident-A path.  The
  // dedicated loop macro marks A loading as already complete instead of
  // relying on pointer arithmetic with a null DRAM address.
  if (A == NULL) {
    gemmini_loop_ws_skip_a(I, J, K, pad_I, pad_J, pad_K, 0, B,
      no_bias ? NULL : D, C, A_row_stride, B_row_stride,
      repeating_bias ? 0 : D_row_stride, C_row_stride,
      a_transpose, b_transpose, full_C, low_D, !no_bias || D == NULL,
...
static void tiled_matmul_outer(size_t dim_I, size_t dim_J, size_t dim_K,
  ...
  gemmini_extended_config_ex(dataflow, act & 7, 0, 1, a_transpose, b_transpose);
  gemmini_extended_config_st(stride_C * sizeof_C, act & 7, scale);
...
static void tiled_norm(const size_t I, const size_t J,
    ...
    gemmini_extended_config_st(J * sizeof(elem_t), act & 7, C_scale);
