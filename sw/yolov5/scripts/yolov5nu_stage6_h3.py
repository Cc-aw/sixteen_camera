#!/usr/bin/env python3
"""Build and validate the YOLOv5nu Stage 6 H3 batched DFL candidate."""
...
ROOT = stage0.ROOT
IMAGE_ID = "025"
STEM = "yolov5nu-stage6-h3-rvvbatchsum-rvvlut-hwsilu-image025-profile"
...
OUTPUT_DIR = ROOT / "fpga/xcvu13p/tests/yolov5nu_stage6_h3"
SMOKE = (
    ROOT / "generators/gemmini/software/gemmini-rocc-tests/build/bareMetalC/"
    "yolov5nu_head_dfl_rvv_batch-baremetal-uart"
)
...
def build_smoke() -> None:
    ...
    run([
        ...
        "yolov5nu_head_dfl_rvv_batch-baremetal",
        "yolov5nu_head_dfl_rvv_batch-baremetal-uart",
    ])
...
def build(python: Path) -> None:
    ...
    environment.update({
        ...
        "YOLOV5NU_HEAD_KERNEL": "optimized-rvv-lut-dfl-batch",
...
def require_disassembly(path: Path, *, full_model: bool) -> dict[str, int]:
    ...
    required = (
        "vredmax_vs", "vfredosum_vs", "vfcvt_x_f_v", "vwmul_vv", "vredsum_vs",
    )
    if full_model:
        required = ("vrgather_vv", *required)
    if any(counts[name] == 0 for name in required):
        raise ValueError(f"{path}: missing Stage 6 H3 RVV instructions: {counts}")
    return counts
...
def source_contract() -> dict[str, int]:
    ...
    contract = {
        ...
        "batch_max_calls": source.count("yolov5nu_rvv_dfl_max_i8x4x16("),
        "ordered_sum_calls": source.count("yolov5nu_rvv_dfl_ordered_sum_f32x4x16("),
        "batch_dot_calls": source.count("yolov5nu_rvv_dfl_probability_dot_i8x4x16("),
    ...
    if contract != expected:
        raise ValueError(f"Stage 6 H3 source contract {contract} != {expected}")
    for marker in (
        "Detection head kernel: optimized-rvv-lut-dfl-batch",
        "YOLOV5NU_HEAD_KERNEL_OPTIMIZED_RVV_LUT_DFL_BATCH 1",
        ...
        if marker not in source and marker not in parameters:
            raise ValueError(f"missing Stage 6 H3 marker: {marker}")
    return contract
...
def validate(python: Path) -> dict:
    ...
    run([
        ...
        "--manifest", str(stage0.MANIFEST), "--source", str(SOURCE),
    ])
    run([
    ...
    run([
        ...
        "--memory-plan", str(MEMORY_PLAN),
    ])
    result = {
        "format": "yolov5nu-stage6-head-h3-build-v1",
        ...
        "image_id": IMAGE_ID,
        "head_kernel": "optimized-rvv-lut-dfl-batch",
