...
SRC="${GEMMINI_DIR}/bareMetalC/yolov5nu_stage3_rvv_smoke.c"
...
case "${TARGET}" in
  fpga)
    OUT="${BUILD_DIR}/yolov5nu-stage3-rvv-smoke-baremetal-uart"
    TARGET_DEFINES=(-DFPGA_MMIO_UART=1 -DFPGA_UART_BASE=0x10020000UL -DFPGA_UART_DIVISOR=434)
    ;;
  sim)
    OUT="${BUILD_DIR}/yolov5nu-stage3-rvv-smoke-baremetal"
    TARGET_DEFINES=()
    ;;
  *)
...
for instruction in vsext.vf2 vfcvt.f.x.v vfcvt.x.f.v vnclip.wi vmax.vv; do
  if ! "${RISCV}/bin/riscv64-unknown-elf-objdump" -d "${OUT}" | grep -q "${instruction}"; then
