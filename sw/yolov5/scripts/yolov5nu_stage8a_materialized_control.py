#!/usr/bin/env python3
"""Build the Stage 8 materialized-Concat control for the five FPGA images."""
...
OUTPUT_DIR = MODEL_DIR / "stage8a_materialized_control"
IMAGE_IDS = ("025", "036", "142", "404", "650")
STAGE8_MODE = "8a-materialized-control"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("command", choices=("build", "elf", "all"), default="all", nargs="?")
...
def source_stem(image_id: str) -> str:
    return f"yolov5nu-stage8a-materialized-control-image{image_id}-profile"
...
def build_source(python: Path, image_id: str) -> str:
    ...
    if contract != expected:
        raise RuntimeError(
            f"invalid materialized control contract: {contract}, expected {expected}"
        )
    (OUTPUT_DIR / f"{stem}_contract.json").write_text(
        json.dumps(contract, indent=2) + "\n"
    )
    print(f"PASS: materialized control {image_id} {contract}")
    return stem
