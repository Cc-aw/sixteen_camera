...
/* Enable per-convolution batch/weight reuse diagnostics at compile time. */
#ifndef GEMMINI_BATCH_DEBUG
#define GEMMINI_BATCH_DEBUG 0
#endif

#ifndef GEMMINI_DISABLE_B_REUSE
#define GEMMINI_DISABLE_B_REUSE 0
#endif
...
#ifndef GEMMINI_CONV_TILE_FENCE
#define GEMMINI_CONV_TILE_FENCE 0
#endif
...
#ifdef GEMMINI_LEGACY_2BIT_ACTIVATION_ABI
#define GEMMINI_ACTIVATION_MASK 3
#else
#define GEMMINI_ACTIVATION_MASK 7
#endif
...
#ifdef GEMMINI_POOL_RUNTIME_DISPATCH
extern unsigned gemmini_pool_active_worker;
#ifdef GEMMINI_POOL_COMMAND_STATS
extern void gemmini_pool_record_command(unsigned funct);
#define GEMMINI_POOL_RECORD_COMMAND(funct) gemmini_pool_record_command(funct)
#else
...
#define ROCC_INSTRUCTION_RS1_RS2(x, rs1, rs2, funct) { \
  ...
  if (gemmini_pool_active_worker == 0) { \
    ...
    ROCC_INSTRUCTION_0_R_R(2, rs1, rs2, funct); \
  } \
}
#else
#define GEMMINI_POOL_RECORD_COMMAND(funct) do { (void)(funct); } while (0)
...
#else
#define gemmini_extended2_config_st(stride, acc_act, acc_scale, pool_stride, pool_size, pool_out_dim, porows, pocols, orows, ocols, upad, lpad) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, ((uint64_t)(ocols) << 56) | ((uint64_t)(orows) << 48) | ((uint64_t)(pocols) << 40) | ((uint64_t)(porows) << 32) | ((uint64_t)(pool_out_dim) << 24) | ((uint64_t)(lpad) << 11) | ((uint64_t)(upad) << 9) | ((uint64_t)(pool_size) << 7) | ((uint64_t)(pool_stride) << 5) | ((uint64_t)(acc_act) << 2) | CONFIG_ST, ((uint64_t)acc_scale_t_to_acc_scale_t_bits((acc_scale_t)acc_scale) << 32) | ((uint32_t)stride), k_CONFIG)
#endif
...
_STATIC void gemmini_config_silu_lut(const elem_t lut[256]) {
  for (uint64_t base = 0; base < 256; base += 8) {
    uint64_t packed = 0;
    for (uint64_t lane = 0; lane < 8; lane++) {
      packed |= (uint64_t)(uint8_t)lut[base + lane] << (lane * 8);
    }
    ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, base, packed, k_CONFIG_SILU_LUT);
  }
}
...
/* IPOAE-A（Author 作者：王志瑞）：LoopConv credit admission 只在显式打开并且硬件导出了专用
   status CSR 的构建中生效。等待体只读取该状态 CSR 并执行 NOP；它不会轮询 busy、
   发射 RoCC、访问内存、刷新 cache 或依赖任何软件 completion。 */
#if defined(GEMMINI_LOOPCONV_CREDIT_ADMISSION) && GEMMINI_LOOPCONV_CREDIT_ADMISSION
#ifndef GEMMINI_LOOPCONV_STATUS_CSR
#error "GEMMINI_LOOPCONV_CREDIT_ADMISSION requires GEMMINI_LOOPCONV_STATUS_CSR"
#endif
#ifndef GEMMINI_LOOPCONV_CREDIT_SAFE_MAX_OVERRIDE
#define GEMMINI_LOOPCONV_CREDIT_SAFE_MAX_OVERRIDE GEMMINI_LOOPCONV_SAFE_MAX
#endif
#if GEMMINI_LOOPCONV_CREDIT_SAFE_MAX_OVERRIDE < 1 || \
    GEMMINI_LOOPCONV_CREDIT_SAFE_MAX_OVERRIDE > GEMMINI_LOOPCONV_SAFE_MAX
#error "GEMMINI_LOOPCONV_CREDIT_SAFE_MAX_OVERRIDE must be within hardware credit capacity"
#endif
#define GEMMINI_LOOPCONV_STATUS_BUSY (1ull << 0)
#define GEMMINI_LOOPCONV_STATUS_CAN_SUBMIT (1ull << 1)
#define GEMMINI_LOOPCONV_STATUS_OUTSTANDING_SHIFT 2
#define GEMMINI_LOOPCONV_STATUS_OUTSTANDING_MASK 0x7ull
#define GEMMINI_LOOPCONV_STATUS_QUEUE_COUNT_SHIFT 8
#define GEMMINI_LOOPCONV_STATUS_QUEUE_COUNT_MASK 0x3ull
#define GEMMINI_LOOPCONV_STATUS_RUNNING_SLOTS_SHIFT 10
#define GEMMINI_LOOPCONV_STATUS_RUNNING_SLOTS_MASK 0x3ull
#define GEMMINI_LOOPCONV_STATUS_ASSEMBLER_PARTIAL (1ull << 12)
#define GEMMINI_LOOPCONV_STATUS_CREDIT_OVERFLOW (1ull << 14)
#define GEMMINI_LOOPCONV_STATUS_CREDIT_UNDERFLOW (1ull << 15)
#ifndef GEMMINI_LOOPCONV_CREDIT_WAIT_HOOK
#define GEMMINI_LOOPCONV_CREDIT_WAIT_HOOK(cycles) do { (void)(cycles); } while (0)
#endif
#ifndef GEMMINI_LOOPCONV_CREDIT_WAIT_TIMEOUT_CYCLES
#define GEMMINI_LOOPCONV_CREDIT_WAIT_TIMEOUT_CYCLES 0ull
#endif
#ifndef GEMMINI_LOOPCONV_CREDIT_WAIT_TIMEOUT_HOOK
#define GEMMINI_LOOPCONV_CREDIT_WAIT_TIMEOUT_HOOK(status, cycles) \
  do { (void)(status); (void)(cycles); } while (0)
#endif
_STATIC inline uint64_t gemmini_loopconv_status_read(void) {
  uint64_t value;
#if defined(GEMMINI_POOL_RUNTIME_DISPATCH) && \
    defined(GEMMINI_LOOPCONV_WORKER1_STATUS_CSR)
  if (gemmini_pool_active_worker != 0) {
    __asm__ volatile("csrr %0, " STR(GEMMINI_LOOPCONV_WORKER1_STATUS_CSR)
                     : "=r"(value) :: "memory");
    return value;
  }
#endif
  __asm__ volatile("csrr %0, " STR(GEMMINI_LOOPCONV_STATUS_CSR)
                   : "=r"(value) :: "memory");
  return value;
}
_STATIC inline uint64_t gemmini_loopconv_cycle_read(void) {
  uint64_t value;
  __asm__ volatile("rdcycle %0" : "=r"(value));
  return value;
}
#ifdef GEMMINI_LOOPCONV_ACCEPTED_CSR
_STATIC inline uint32_t gemmini_loopconv_accepted_count_read(void) {
  uint64_t value;
#if defined(GEMMINI_POOL_RUNTIME_DISPATCH) && \
    defined(GEMMINI_LOOPCONV_WORKER1_ACCEPTED_CSR)
  if (gemmini_pool_active_worker != 0) {
    __asm__ volatile("csrr %0, " STR(GEMMINI_LOOPCONV_WORKER1_ACCEPTED_CSR)
                     : "=r"(value) :: "memory");
    return (uint32_t)value;
  }
#endif
  __asm__ volatile("csrr %0, " STR(GEMMINI_LOOPCONV_ACCEPTED_CSR)
                   : "=r"(value) :: "memory");
  return (uint32_t)value;
}
#endif
#ifdef GEMMINI_LOOPCONV_RETIRED_CSR
_STATIC inline uint32_t gemmini_loopconv_retired_count_read(void) {
  uint64_t value;
#if defined(GEMMINI_POOL_RUNTIME_DISPATCH) && \
    defined(GEMMINI_LOOPCONV_WORKER1_RETIRED_CSR)
  if (gemmini_pool_active_worker != 0) {
    __asm__ volatile("csrr %0, " STR(GEMMINI_LOOPCONV_WORKER1_RETIRED_CSR)
                     : "=r"(value) :: "memory");
    return (uint32_t)value;
  }
#endif
  __asm__ volatile("csrr %0, " STR(GEMMINI_LOOPCONV_RETIRED_CSR)
                   : "=r"(value) :: "memory");
  return (uint32_t)value;
}
#endif
_STATIC inline void gemmini_loopconv_wait_for_credit(void) {
  uint64_t wait_start = 0;
  int waiting = 0;
  int timeout_reported = 0;
  for (;;) {
    const uint64_t status = gemmini_loopconv_status_read();
    const uint64_t outstanding =
        (status >> GEMMINI_LOOPCONV_STATUS_OUTSTANDING_SHIFT) &
        GEMMINI_LOOPCONV_STATUS_OUTSTANDING_MASK;
    if ((status & GEMMINI_LOOPCONV_STATUS_CAN_SUBMIT) &&
        outstanding < GEMMINI_LOOPCONV_CREDIT_SAFE_MAX_OVERRIDE) {
      GEMMINI_LOOPCONV_CREDIT_WAIT_HOOK(
          waiting ? gemmini_loopconv_cycle_read() - wait_start : 0);
      return;
    }
    if (!waiting) {
      wait_start = gemmini_loopconv_cycle_read();
      waiting = 1;
    }
    if (!timeout_reported && GEMMINI_LOOPCONV_CREDIT_WAIT_TIMEOUT_CYCLES != 0) {
      const uint64_t waited = gemmini_loopconv_cycle_read() - wait_start;
      if (waited >= GEMMINI_LOOPCONV_CREDIT_WAIT_TIMEOUT_CYCLES) {
        GEMMINI_LOOPCONV_CREDIT_WAIT_TIMEOUT_HOOK(status, waited);
        timeout_reported = 1;
      }
    }
...
#else
#define gemmini_loopconv_wait_for_credit() do { } while (0)
#endif
...
#define gemmini_loop_conv_ws(batch_size, in_row_dim, in_col_dim, in_channels, out_channels, out_row_dim, out_col_dim, pool_out_row_dim, pool_out_col_dim, stride, padding, kernel_dim, kernel_dilation, pool_size, pool_stride, pool_padding, batches, porows, pocols, pochs, krows, kcols, kchs, lpad, rpad, upad, dpad, plpad, prpad, pupad, pdpad, orows, ocols, weights, output, bias, input, no_bias, no_pool, downsample, wrot180, input_dilated, activation, trans_output_1203, trans_weight_1203, trans_weight_0132, trans_input_3120, max_pixels_per_row, in_stride, weight_stride, out_stride, dw, a_spad_id, b_spad_id) \
  { \
    gemmini_loopconv_wait_for_credit(); \
...
static void tiled_matmul_outer(size_t dim_I, size_t dim_J, size_t dim_K,
  ...
  gemmini_extended_config_ex(dataflow, act & GEMMINI_ACTIVATION_MASK, 0, 1, a_transpose, b_transpose);
  gemmini_extended_config_st(stride_C * sizeof_C, act & GEMMINI_ACTIVATION_MASK, scale);
...
#ifdef GEMMINI_POOL_NO_INTERNAL_FENCE
  asm volatile("" ::: "memory");
#else
  gemmini_fence();
#endif
...
#if GEMMINI_BATCH_DEBUG
    const size_t weight_tiles = num_kch * num_poch * num_krow * num_kcol;
    printf("batch_size=%d batches=%d num_b=%d b_reuse=%d weight_tiles=%d\n",
           batch_size, batches, (int)num_b, (int)b_reuse,
           (int)weight_tiles);
#endif
...
_STATIC void tiled_conv_dw_auto(
    ...
    // Direct tile size calculation for depthwise conv
    // Key constraints:
    //   spad_rows = batches * irows * icols + krows * kcols  (kchs=1, ochs=1)
    //   acc_rows  = batches * orows * ocols
    //   irows = orows * stride + kcols - 1
    //   icols = ocols * stride + krows - 1
    // We prioritize ocols = DIM for full systolic array utilization,
    // then calculate max orows from acc constraint.

    int krows = kernel_dim;
    int kcols = kernel_dim;

    // Phase 1: aim ocols = DIM for full systolic array utilization
    int ocols = pool_out_col_dim;
    if (ocols > DIM) ocols = DIM;

    // Phase 2: start with batches=1, max orows from acc constraint
    int batches = 1;
    int orows = max_acc_rows / (batches * ocols);
    if (orows > pool_out_row_dim) orows = pool_out_row_dim;

    // Phase 3: verify spad constraint
    int spad_rows = tiled_conv_total_spad_rows(false,
        stride, 1, 1, false, false, false,
        batches, orows, ocols, 1, krows, kcols, 1, pool_size, pool_stride);

    while ((spad_rows > max_spad_rows || orows * ocols > max_acc_rows) && (orows > 1 || ocols > 1)) {
        if (orows > ocols && orows > 1) orows--;
        else if (ocols > 1) ocols--;
        spad_rows = tiled_conv_total_spad_rows(false,
            stride, 1, 1, false, false, false,
            batches, orows, ocols, 1, krows, kcols, 1, pool_size, pool_stride);
    }

    // Phase 4: if orows is limited by pool_out_row_dim, try growing ocols
    if (orows == pool_out_row_dim && orows > 1) {
        int max_ocols = max_acc_rows / (batches * orows);
        if (max_ocols > ocols) {
            int new_ocols = max_ocols < pool_out_col_dim ? max_ocols : pool_out_col_dim;
            int new_spad = tiled_conv_total_spad_rows(false,
                stride, 1, 1, false, false, false,
                batches, orows, new_ocols, 1, krows, kcols, 1, pool_size, pool_stride);
            if (new_spad <= max_spad_rows) {
                ocols = new_ocols;
            }
        }
    }

    // Phase 5: try to increase batches for better throughput (if spad allows)
    while (batches < batch_size) {
        int next_batches = batches + 1;
        int next_acc = next_batches * orows * ocols;
        if (next_acc > max_acc_rows) break;
        int next_spad = tiled_conv_total_spad_rows(false,
            stride, 1, 1, false, false, false,
            next_batches, orows, ocols, 1, krows, kcols, 1, pool_size, pool_stride);
        if (next_spad > max_spad_rows) break;
    ...
    printf("depthwise tile: batches=%d orows=%d ocols=%d krows=%d kcols=%d  spad=%d/%d acc=%d/%d\n",
        batches, orows, ocols, krows, kcols,
        spad_rows, max_spad_rows, acc_rows, max_acc_rows);
...
static void tiled_norm(const size_t I, const size_t J,
    ...
    gemmini_extended_config_st(J * sizeof(elem_t), act & GEMMINI_ACTIVATION_MASK, C_scale);
