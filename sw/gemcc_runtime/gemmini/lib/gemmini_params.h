...
/* DIM*sizeof(acc_t) may exceed MAX_BYTES; keep accumulator block length safe. */
...
/* This board uses a Rocket 64-set, 8-way, 64-byte D-cache (32 KiB).
 * Gemmini and the framebuffer DMA access the same DDR through the physical
 * alias, so the library backend must evict the complete L1 at CPU/Gemmini
 * ownership boundaries.  These values are for the DIM16 Taihang bitstream;
 * do not reuse the historical DIM32 128 KiB profile. */
#define GEMCC_COHERENCE_BACKEND_MYBOARD 1
#define GEMCC_COHERENCE_BACKEND_DECLARED 1
#define GEMCC_MYBOARD_CACHE_BYTES (32u * 1024u)
#define GEMCC_MYBOARD_CACHE_LINE 64u
#define GEMCC_DMA_COHERENT 0
