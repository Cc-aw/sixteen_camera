...
/* MyBoard (184): 3-bit acc_act, pool_stride<<5, pool_size<<7, upad<<9, lpad<<11.
 * Stock Berkeley is pool_stride<<4 / pool_size<<6 — wrong on this bitstream. */
...
/* Bare-metal builds do not link libc; keep this helper local to the Gemmini
 * implementation instead of relying on stdlib's external abs symbol. */
static inline acc_t gemmini_abs_acc(acc_t x) {
  return x < 0 ? -x : x;
}
...
static elem_t scale_and_sat(acc_t x, int act, acc_scale_t scale, acc_scale_t bert_scale) {
  ...
  if (act == IGELU) {
    ...
    const acc_t q_abs = gemmini_abs_acc(q);
    const acc_t q_clipped = q_abs > (-qb) ? (-qb) : q_abs;
...
static void tiled_conv_stride_auto(
    ...
    while (!nothing_increased) {
        ...
        for (size_t i = 0; i < sizeof(args)/sizeof(args[0]); i++) {
            int args_candidate[] = {args[0], args[1], args[2], args[3], args[4], args[5], args[6]};
            // Channel tiles must remain DIM-aligned.  Allowing the greedy
            // growth pass to add one produces kchs=126 for the 40->20
            // stride-2 convolution, which is not a legal WS tile on DIM32.
#ifdef GEMCC_CHANNEL_TILE_DIM
            args_candidate[i] +=
                (i == out_channels_idx || i == in_channels_idx)
                    ? GEMCC_CHANNEL_TILE_DIM : 1;
#else
            args_candidate[i]++;
#endif
