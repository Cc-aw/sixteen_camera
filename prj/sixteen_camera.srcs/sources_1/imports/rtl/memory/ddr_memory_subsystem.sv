...
module ddr_memory_subsystem (
    ...
    design_1_wrapper u_ddr_bd (
        ...
        // Preprocess frame reads stay on DDR S02.  Its tensor writes instead
        // cross to the SoC FBus below, so Gemmini observes coherent data.
        .S02_AXI_awid(3'd0),
    ...
    axi4_write_cdc #(.FIFO_ADDR_WIDTH(4)) u_preprocess_fbus_cdc (
        .s_axi(preprocess_write_axi), .m_clk(soc_clk),
