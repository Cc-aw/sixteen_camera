...
set camera_clocks [get_clocks -quiet {
    cam_pclk_1 cam_pclk_2 cam_pclk_3 cam_pclk_4
    cam_pclk_5 cam_pclk_6 cam_pclk_7 cam_pclk_8
}]
...
foreach cell $iob_cells {
    if {![string equal -nocase [get_property -quiet IOB $cell] "true"]} {
        lappend bad_iob_cells $cell
    }
}
puts "CAMERA_IOB_SAMPLE_CELLS=[llength $iob_cells]"
puts "CAMERA_IOB_BAD_CELLS=[llength $bad_iob_cells]"
if {[llength $iob_cells] != 88 || [llength $bad_iob_cells] != 0} {
    error "camera input sample registers are not fully packed into IOBs"
}

puts "ROUTED_CAMERA_IOB_CHECK=PASS"
close_design
close_project
